...
def periodic_experiment(error, max_steps_window: int = 17):
    ...
    for country in COUNTRIES:
        print(f"Running periodic experiment for {country} in {year} with error {error}.")
        ci = _load_dataset(f"data/{country}_{year}_ci.csv")
    ...
    if not os.path.exists("results"):
        os.makedirs("results")
    with open(f"results/{year}_periodic_{error}.csv", "w") as csvfile:
...
def ad_hoc_experiment(experiment_name: str,
    ...
    with open(f"results/{year}_{experiment_name}_{forecast_method}{e}{i}_{country}.csv", "w") as csvfile:
        df.to_csv(csvfile)
...
def generate_batch_jobs(n: int,
    ...
    # Include the full last day up to 23:30
    steps = pd.date_range("2025-01-01 00:00", "2025-12-31 23:30", freq="30min")
    ...
    # Same here: full days, but this is fine as-is
